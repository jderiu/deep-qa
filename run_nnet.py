...
import deepnn.layers as layers
...
def main():
  ...
  q_train = numpy.load(os.path.join(data_dir, '{}.questions.npy'.format(mode.lower())))
  a_train = numpy.load(os.path.join(data_dir, '{}.answers.npy'.format(mode.lower())))
  q_overlap_train = numpy.load(os.path.join(data_dir, '{}.q_overlap_indices.npy'.format(mode.lower())))
  a_overlap_train = numpy.load(os.path.join(data_dir, '{}.a_overlap_indices.npy'.format(mode.lower())))
  y_train = numpy.load(os.path.join(data_dir, '{}.labels.npy'.format(mode.lower())))
  ...
  ###### QUESTION ######
  lookup_table_words = layers.LookupTableFastStatic(W=vocab_emb, pad=max(q_filter_widths)-1)
  lookup_table_overlap = layers.LookupTableFast(W=vocab_emb_overlap, pad=max(q_filter_widths)-1)

  lookup_table = layers.ParallelLookupTable(layers=[lookup_table_words, lookup_table_overlap])
  ...
  for filter_width in q_filter_widths:
    ...
    conv = layers.Conv2dLayer(rng=numpy_rng, filter_shape=filter_shape, input_shape=input_shape)
    non_linearity = layers.NonLinearityLayer(b_size=filter_shape[0], activation=activation)
    pooling = layers.KMaxPoolLayer(k_max=q_k_max)
    conv2dNonLinearMaxPool = layers.FeedForwardNet(layers=[conv, non_linearity, pooling])
    conv_layers.append(conv2dNonLinearMaxPool)

  join_layer = layers.ParallelLayer(layers=conv_layers)
  flatten_layer = layers.FlattenLayer()

  nnet_q = layers.FeedForwardNet(layers=[
                                  lookup_table,
                                  join_layer,
                                  flatten_layer,
                                  ])
  ...
  for filter_width in a_filter_widths:
    ...
    conv = layers.Conv2dLayer(rng=numpy_rng, filter_shape=filter_shape, input_shape=input_shape)
    non_linearity = layers.NonLinearityLayer(b_size=filter_shape[0], activation=activation)
    pooling = layers.KMaxPoolLayer(k_max=a_k_max)
    conv2dNonLinearMaxPool = layers.FeedForwardNet(layers=[conv, non_linearity, pooling])
    conv_layers.append(conv2dNonLinearMaxPool)
  ...
  nnet_a = layers.FeedForwardNet(layers=[
                                  lookup_table,
                                  join_layer,
                                  flatten_layer,
                                  ])
  ...
  # dropout_q = layers.FastDropoutLayer(rng=numpy_rng)
  # dropout_a = layers.FastDropoutLayer(rng=numpy_rng)
  # dropout_q.set_input(nnet_q.output)
  # dropout_a.set_input(nnet_a.output)

  # feats_nout = 10
  # x_hidden_layer = layers.LinearLayer(numpy_rng, n_in=feats_ndim, n_out=feats_nout, activation=activation)
  # x_hidden_layer.set_input(x)

  # feats_nout = feats_ndim

  ### Dropout
  # classifier = layers.PairwiseLogisticWithFeatsRegression(q_in=logistic_n_in,
  #                                                   a_in=logistic_n_in,
  #                                                   n_in=feats_nout,
  #                                                   n_out=n_outs)
  ...
  # # train_nnet = layers.FeedForwardNet(layers=[nnet_q, nnet_a, x_hidden_layer, dropout_q, dropout_a, classifier],
  # train_nnet = layers.FeedForwardNet(layers=[nnet_q, nnet_a, dropout_q, dropout_a, classifier],
  #                                       name="Training nnet")

  # test_classifier = layers.PairwiseLogisticWithFeatsRegression(q_in=logistic_n_in,
  #                                                         a_in=logistic_n_in,
  #                                                         n_in=feats_nout,
  #                                                         n_out=n_outs,
  #                                                         W=classifier.W,
  ...
  # # test_nnet = layers.FeedForwardNet(layers=[nnet_q, nnet_a, x_hidden_layer, test_classifier],
  # test_nnet = layers.FeedForwardNet(layers=[nnet_q, nnet_a, test_classifier],
  #                                       name="Test nnet")
  #########

  # pairwise_layer = layers.PairwiseMultiOnlySimWithFeatsLayer(q_in=q_logistic_n_in,

  pairwise_layer = layers.PairwiseNoFeatsLayer(q_in=q_logistic_n_in,
  # pairwise_layer = layers.PairwiseWithFeatsLayer(q_in=q_logistic_n_in,
  # pairwise_layer = layers.PairwiseOnlySimWithFeatsLayer(q_in=q_logistic_n_in,
  ...
  hidden_layer = layers.LinearLayer(numpy_rng, n_in=n_in, n_out=n_in, activation=activation)
  hidden_layer.set_input(pairwise_layer.output)

  classifier = layers.LogisticRegression(n_in=n_in, n_out=n_outs)
  classifier.set_input(hidden_layer.output)


  train_nnet = layers.FeedForwardNet(layers=[nnet_q, nnet_a, pairwise_layer, hidden_layer, classifier],
  # train_nnet = layers.FeedForwardNet(layers=[nnet_q, nnet_a, x_hidden_layer, classifier],
                                        name="Training nnet")
  ...
  train_set_iterator = trainers.MiniBatchIteratorConstantBatchSize(numpy_rng, [q_train, a_train, q_overlap_train, a_overlap_train, y_train], batch_size=batch_size, randomize=True)
  dev_set_iterator = trainers.MiniBatchIteratorConstantBatchSize(numpy_rng, [q_dev, a_dev, q_overlap_dev, a_overlap_dev, y_dev], batch_size=batch_size, randomize=False)
